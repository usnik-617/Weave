  ...
  if (opsTelemetryResetBtn) {
    opsTelemetryResetBtn.addEventListener('click', async () => {
      ...
      if (typeof notifyInfo === 'function') {
        notifyInfo('?대씪?댁뼵???붾젅硫뷀듃由?移댁슫?곕? 珥덇린?뷀뻽?듬땲??');
      }
    });
  }
...
function initWriteEntryBindingsFromGlobals() {
  ...
  if (newsWriteBtn) {
    newsWriteBtn.addEventListener('click', () => {
      ...
      if (!user || user.status !== 'active' || !isStaffUser(user)) {
        notifyMessage('?댁쁺吏?愿由ъ옄(?뺤떇 ?⑥썝)留?怨듭? ?묒꽦??媛?ν빀?덈떎.');
        return;
      }
      resetWriteForms();
  ...
  if (faqWriteBtn) {
    faqWriteBtn.addEventListener('click', () => {
      const user = getCurrentUser();
      if (!isAdminUser(user)) {
        notifyMessage('FAQ ?묒꽦? 愿由ъ옄留?媛?ν빀?덈떎.');
        return;
      }
      resetWriteForms();
      const form = document.getElementById('add-news-form');
  ...
  if (qnaWriteBtn) {
    qnaWriteBtn.addEventListener('click', () => {
      ...
      if (!user || user.status !== 'active') {
        notifyMessage('?뺤떇 ?⑥썝 ?뱀씤 ??Q&A ?묒꽦??媛?ν빀?덈떎.');
        return;
      }
      resetWriteForms();
      const form = document.getElementById('add-news-form');
  ...
  if (galleryWriteBtn) {
    galleryWriteBtn.addEventListener('click', () => {
      const user = getCurrentUser();
      if (!user || user.status !== 'active' || !isStaffUser(user)) {
        notifyMessage('?댁쁺吏?愿由ъ옄(?뺤떇 ?⑥썝)留?媛ㅻ윭由??낅줈?쒓? 媛?ν빀?덈떎.');
        return;
      }
      resetWriteForms();
