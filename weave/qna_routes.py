...
def create_qna_post(payload, conn, me, post_visibility_status):
    ...
    log_audit(conn, "create_post", "post", post_id, me["id"], {"category": "qna"})
    from weave.core import record_user_activity

    record_user_activity(
        conn, me["id"], "post_create", "post", post_id, {"category": "qna"}
    )
    return post_id, None


def update_qna_post(post_id, payload, conn, me, post_visibility_status):
...
def delete_qna_post(post_id, conn, me):
