...
def check_files(base_dir: Path, checks: CheckCollector):
    required_files = ['app.py', 'wsgi.py', 'requirements.txt', 'README.md']
...
def check_production_env(effective_env: str, checks: CheckCollector):
    ...
    if effective_env != 'production':
        checks.warn(f'WEAVE_ENV가 production이 아닙니다. (현재: {effective_env})')
        return
    ...
    if not trusted_hosts:
        ...
        if any(h in {'127.0.0.1', 'localhost'} for h in hosts):
            checks.warn('WEAVE_TRUSTED_HOSTS에 로컬 호스트가 포함되어 있습니다.')
        checks.ok(f'WEAVE_TRUSTED_HOSTS 설정 확인 ({len(hosts)}개)')
    ...
    try:
        ...
        if proxy_hops >= 1:
            ...
            checks.fail('WEAVE_PROXY_HOPS는 1 이상이어야 합니다.')
    except ValueError:
        checks.fail('WEAVE_PROXY_HOPS가 정수가 아닙니다.')
...
def check_default_admin_password(base_dir: Path, effective_env: str, checks: CheckCollector):
    ...
    if DEFAULT_ADMIN_PASSWORD in app_py:
        if effective_env == 'production':
            checks.fail('코드에 기본 관리자 시드 비밀번호 문자열이 남아 있습니다.')
        else:
            checks.warn('코드에 기본 관리자 시드 비밀번호 문자열이 존재합니다.')
    else:
...
def check_db(base_dir: Path, checks: CheckCollector):
    ...
    if not db_path.exists():
        checks.warn('weave.db 파일이 아직 없습니다. (최초 실행 전일 수 있습니다.)')
        return
    try:
        ...
        if str(mode).lower() == 'wal':
            ...
            checks.warn(f'SQLite journal_mode가 WAL이 아닙니다. ({mode})')
    except Exception as exc:
        checks.fail(f'SQLite 점검 실패: {exc}')


def check_runtime_separation(checks: CheckCollector):
    report = validate_runtime_separation()
    checks.ok(f"DB 모드: {report['db_mode']}")
    checks.ok(f"스토리지 백엔드: {report['storage_backend']}")
    checks.ok(f"미디어 큐 백엔드: {report['queue_backend']}")
    ...
    try:
        with urllib.request.urlopen(health_url, timeout=5) as response:
            ...
            if response.status == 200 and isinstance(payload, dict):
                ...
                checks.fail(f'헬스체크 비정상 응답: {health_url}')
    except urllib.error.URLError as exc:
        checks.fail(f'헬스체크 접속 실패: {health_url} ({exc})')
    except json.JSONDecodeError:
        checks.fail(f'헬스체크 응답이 JSON이 아닙니다: {health_url}')


def main():
    parser = argparse.ArgumentParser(description='Weave 운영 전 사전 점검')
    ...
    parser.add_argument('--start-local', action='store_true', help='점검 전에 로컬 Waitress 서버를 임시 실행')
    ...
    check_runtime_separation(checks)

    server_proc = None
    if args.start_local:
        python_exe = Path(sys.executable)
        cmd = [str(python_exe), '-m', 'waitress', '--host=127.0.0.1', '--port=5000', 'app:app']
